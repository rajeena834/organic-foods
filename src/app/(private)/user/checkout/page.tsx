...
import productsCartStore, {
  IProductsCartStore,
} from "@/globals-store/products-cart-store";
...
import { getStripePaymentIntentToken } from "@/actions/payment";
...
function UserCheckoutPage() {
  ...
  const onPaymentSuccess = async (paymentId: string) => {
    try {
      ...
      console.log("response from save order", response);
      if (!response.success) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex justify-center items-center gap-5">
        ...
        <Button 
         disabled={!selectedAddress || loading} onClick={onCheckout}
        >
          Proceed to payment
        </Button>
      </div>
